...
def _cached_model_scan_script(model_dirs: list[str] | None = None) -> str:
    ...
    lines = [
        ...
        "    env_base = (os.environ.get('OLLAMA_BASE_URL') or '').strip().rstrip('/')",
        "    if env_base.endswith('/v1'): env_base = env_base[:-3].rstrip('/')",
        "    if env_base.startswith('http'): urls.insert(0, env_base + '/api/tags')",
        "    for url in urls:",
        "        try:",
