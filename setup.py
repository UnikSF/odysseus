...
def generate_pwa_icons():
    """Generate static/icon-192.png and static/icon-512.png if missing."""
    import struct, zlib
    static_dir = os.path.join(BASE_DIR, "static")

    def _solid_png(size: int, r: int, g: int, b: int) -> bytes:
        def chunk(tag, data):
    ...
    for size, name in [(192, "icon-192.png"), (512, "icon-512.png")]:
        ...
        if os.path.exists(path):
            print(f"  [skip] static/{name} already exists")
            continue
        try:
            import cairosvg
            svg_src = os.path.join(static_dir, "icon.svg")
            cairosvg.svg2png(url=svg_src, write_to=path, output_width=size, output_height=size)
            print(f"  [ok] static/{name} (cairosvg)")
        except Exception:
            with open(path, "wb") as f:
                f.write(_solid_png(size, 40, 44, 52))
            print(f"  [ok] static/{name} (solid color fallback)")
