...
# ========= ROOT PATH (subpath deployment) =========
# Set ROOT_PATH=/odysseus (no trailing slash) when Odysseus is served under a
# subpath by a reverse proxy. This rewrites HTML asset URLs and redirects so
# the browser stays on the correct prefix.
...
app.include_router(setup_benchmark_routes(session_manager))
...
from routes.agent_route_routes import setup_agent_route_routes
app.include_router(setup_agent_route_routes())
...
def _serve_html_with_nonce(request: Request, file_path: str) -> HTMLResponse:
    ...
    if ROOT_PATH:
        ...
        patch = (
            ...
            f'(function(){{var R="{ROOT_PATH}";'
            f'function _p(u){{return typeof u==="string"&&u.startsWith("/")&&!u.startsWith(R)?R+u:u;}}'
            f'var f=window.fetch;'
            f'window.fetch=function(u,o){{return f.call(this,_p(u),o);}};'
            f'var E=window.EventSource;'
            f'window.EventSource=function(u,o){{return new E(_p(u),o);}};'
            f'var lr=window.location.replace.bind(window.location);'
            f'window.location.replace=function(u){{lr(_p(u));}};'
            f'var la=window.location.assign.bind(window.location);'
            f'window.location.assign=function(u){{la(_p(u));}};'
            f'}})();</script>'
        )
