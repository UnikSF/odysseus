...
function extractMerchant(desc: string): string {
  ...
  let m = desc.replace(prefixes, "").trim().replace(/\s+\d{2}\/\d{2}(\s+.*)?$/, "").trim();
...
// e.g. "24.0427.045,00"  →  op=24/04  val=27/04  amount=5,00
...
function importRows(rows: RawTx[]): { imported: number; skipped: number; errors: number } {
  const db = getDb();
  ...
  for (const { date, description, amount } of rows) {
    try {
      ...
      const result = insert.run(
        randomUUID(), date, amount, merchant, description,
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ ...counts, warning: parseError });
}
